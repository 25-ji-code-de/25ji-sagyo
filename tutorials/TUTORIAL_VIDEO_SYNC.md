...
整个 24 小时通常被分为若干个 Part，用于映射到不同的视频片段。当前实现支持多种播放模式：

* `mp4-3`：3 个 Part，每个 Part 长度为 8 小时（典型模式，对应 `p1`、`p2`、`p3`）。
* `mp4-6`：6 个 Part，每个 Part 长度为 4 小时（对应 `p1`..`p6`），可用于将全天细分为更多片段。
* `hls`：使用 HLS（m3u8）源，播放器会按全天偏移（从凌晨 01:00 起的秒数）进行定位。

系统会根据可用的视频源自动选择播放模式（例如若存在 p4-p6 源则使用 `mp4-6`，若存在 m3u8 则优先使用 `hls`），并根据当前时间计算应该播放的 Part 及在该 Part 中的偏移秒数，然后加载并跳转到精确的位置。
...
*   **初始化同步**：网页加载时，会自动计算时间并加载正确的视频片段或 HLS 偏移点。
*   **定期校准**：播放器实现会每 5 秒运行一次同步检查（实现中使用 setInterval 5000ms），并使用 30 秒作为漂移阈值（如果播放进度与期望偏差超过 30 秒，则会进行跳转/seek）。
*   **跨段自动切换**：当时间从一个 Part 跨越到下一个 Part（例如从 08:59:59 到 09:00:00），系统会自动加载下一个视频文件并无缝播放；在 HLS 模式下，若 HLS 初始化失败，播放器会回退到 MP4 模式并重试同步。
